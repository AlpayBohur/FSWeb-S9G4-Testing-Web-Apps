...
test("kullanıcı adını 5 karakterden az girdiğinde BİR hata mesajı render ediyor.", async () => {
  ...
  userEvent.type(kullaniciAdi, "alp");
...
test("kullanıcı doğru ad ve soyad girdiğinde ama email girmediğinde BİR hata mesajı render ediliyor.", async () => {
  ...
  userEvent.type(kullaniciAdi, "alpay");
  userEvent.type(kullaniciSoyad, "bohur");
  userEvent.click(btnSubmit);
...
test("ad,soyad, email render ediliyor. mesaj bölümü doldurulmadığında hata mesajı render edilmiyor.", async () => {
  ...
  userEvent.type(kullaniciAdi, "alpay");
  userEvent.type(kullaniciSoyad, "bohur");
  userEvent.type(kullaniciEmail, "alpay@mail.com");
  userEvent.click(btnSubmit);
...
test("form gönderildiğinde girilen tüm değerler render ediliyor.", async () => {
  ...
  userEvent.type(kullaniciSoyad, "bohur");
  userEvent.type(kullaniciEmail, "alpay@mail.com");
  userEvent.type(msg, "yazdım kaç gün oldu...");
  userEvent.click(btnSubmit);

  await waitFor(() => {
    const msgInput = screen.queryByTestId("messageDisplay");
    expect(msgInput).toBeInTheDocument();
    expect(msgInput.textContent).toMatch(/yazdım kaç gün oldu.../i);
  });
});
